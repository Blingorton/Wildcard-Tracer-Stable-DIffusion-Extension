...
class _ResolutionRecorder:
    def __init__(self):
        self._patched  = False
        # Start collecting immediately at construction time — before any
        # before_process hook fires — so records are never dropped due to
        # extension load-order races with sd-dynamic-prompts.
        self._records  = []
        self._lock     = threading.Lock()

    def start(self):
        """Reset for a new generation. Safe to call before or after patching."""
        with self._lock:
            self._records = []
...
class WildcardTracerScript(scripts.Script):
    ...
    def before_process(self, p, *args):
        RECORDER.start()
        # Prefer the "Template" key that sd-dynamic-prompts writes to
        # extra_generation_params (the raw pre-resolution prompt).  Fall back
        # to p.prompt only if that key isn't present yet — which can happen
        # if our before_process runs before sd-dynamic-prompts'.  In that case
        # p.prompt is still the raw template anyway, so the fallback is safe.
        params = getattr(p, "extra_generation_params", {}) or {}
        self._wt_pos_template = (
            params.get("Template", "") or
            (getattr(p, "prompt", "") or "")
        ).strip()
    ...
    def process_before_every_sampling(self, p, *args, **kwargs):
        ...
        # prompt) when available — this is reliable regardless of hook order.
        # The captured self._wt_pos_template is a fallback for when Template
        # isn't written yet (e.g. sd-dynamic-prompts disabled).
        pos_template = (params.get("Template", "") or self._wt_pos_template or "").strip()
        # If the template was captured after sd-dynamic-prompts already resolved
        # p.prompt, self._wt_pos_template may equal the resolved text.  In that
        # case the Template param (if present) is the only reliable source.
        if pos_template == resolved and params.get("Template"):
            pos_template = params["Template"].strip()
